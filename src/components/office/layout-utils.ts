...
/**
 * Returns a random [x, z] point within the office floor bounds,
 * inset by the given margin to keep robots away from walls.
 */
export function getRandomFloorPoint(
  floorWidth: number,
  floorDepth: number,
  margin = 1,
): [number, number] {
  const hw = floorWidth / 2 - margin;
  const hd = floorDepth / 2 - margin;
